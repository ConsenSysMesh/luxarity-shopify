...
async function getorderid(){
  ...
  var fetchUrl = 'https://00000000000000000000000000000000:11111111111111111111111111111111@luxarity-popup-2016.myshopify.com/admin/orders.json?since_ids=776273756203';
            ...
            if(!(json.orders[i].id && json.orders[i].total_price && json.orders[i].order_number && json.orders[i].customer)){
                ...
                try{
                 //sendsqs(ipfsURL, json.orders[i].id, json.orders[i].total_price, json.orders[i].order_number, customerEmail256, json.orders[i].customer.email, redemptionPin256);
                  doinsertTmpOrder(ipfsURL, json.orders[i].id, json.orders[i].total_price, json.orders[i].order_number, customerEmail256, json.orders[i].customer.email, redemptionPin256)
                  console.log("sent insert to db: "+json.orders[i].id)
...
async function getLastOrderId() {
  ...
  async function doinsertTmpOrder(ipfsURL, orderid, total_price, order_number, customerEmail256, customeremail, redemptionPin256){
    await insertTmpOrder(ipfsURL, orderid, total_price, order_number, customerEmail256, customeremail, redemptionPin256)
    console.log("done insertTmpOrder: "+orderid)
  }

  async function insertTmpOrder(ipfsURL, orderid, total_price, order_number, customerEmail256, customeremail, redemptionPin256) {

  console.log("inside insertTmpOrder : ");
  ...
  const query = {
    name: 'insertTmpOrder',
    text: "insert into tmp_orders_copy (tokenuri, orderid, totalcost, ordernumber, customeremail256, customeremail, redemptionhash ) VALUES ($1,$2,$3,$4,$5,$6,$7)",
    values: [ipfsURL, orderid, total_price, order_number, customerEmail256, customeremail, redemptionPin256]
  }
